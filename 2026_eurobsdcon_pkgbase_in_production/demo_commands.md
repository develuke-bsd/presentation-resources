...
git clone https://github.com/freebsd/freebsd-src.git /usr/src
cd /usr/src
git checkout release/15.1.0
poudriere jail -c -B -j 15base -m src=/usr/src -b -v 15.1-RELEASE -K GENERIC
git checkout release/15.1.0-p1
...
```
poudriere jail -c -j 15base -v release/15.1.0 -B -a amd64 -D -K GENERIC -m git+https
poudriere jail -u -j 15base -t release/15.1.0p1
```
repeat with every patch level
Results in /usr/local/poudriere/data/images/15base-repo/FreeBSD:15:amd64/
## Publish the results to the Network
...
```
When using Poudriere, use `alias /usr/local/poudriere/data/images/15base-repo/`
...
zfs create zroot/jails/pkgbase
```
